<?php 

$i18n = [

	"INVITATION" 		=> "Dołącz zaproszenie do newslettera w swoim szablonie",
	"INVITATION_SHORTCODE" => "Lub użyj następującego krótkiego kodu w edytorze",
	"SENDNEWSLETTER" 	=> "Wyślij Newsletter",
	"MAILINGLIST"		=> "Lista Mailingowa & Ustawienia",

	"MESSAGESUCCESS"	=> "Wiadomość została wysłana!",
	"MESSAGEERROR"		=> "Wiadomość nie została wysłana!",
	"NEWSLETTERTITLE"	=> "Tytuł Newslettera:",
	"NEWSLETTERCONTENT"	=> "Treść Newslettera:",
	"SENDNEWNEWSLETTER"	=> "Wyślij Nowy Newsletter",
	"SEND"				=> "Wyślij",

	"NEWSLETTERSETTINGS" => "Ustawienia Newsletter'a",
	"SMTPSENDER"		=> "SMTP Adres e-mail nadawcy:",
	"SMTPSENDER"		=> "SMTP Adres e-mail nadawcy:",
	"SMTPMAIL"			=> "Email Nadawcy",
	"SENDERNAME"		=> "Nazwa Nadawcy:",
	"SENDERNAMEPLACEHOLDER" => "John Doe",
	"SMTPPASSWORD"		=> "SMTP Hasło:",
	"SMTPSERVER"		=> "SMTP Server:",
	"SMTPSERVERPLACEHOLDER" => "mail.server.com",
	"SMTPPORT"			=> "SMTP Port:",
	"REQAUTH"			=> "Autoryzacja wymagana?",
	"REQSSL"			=> "Użyć SSL?",
	"YES"				=> "Tak",
	"NO"				=> "Nie",

	"NEWSLETTERECP"		=> "Odbiorcy Newslettera:",
	"NEWSLETTERECPSMALL" => "Adresy są oddzielone przecinkami. (<span style='color:red;'>Ostatni przecinek jest wymagany! Nie usuwaj go!</span>) ",

	"SUBSUCCESS"		=> "Sukces, informacje o subskrypcji",
	"SUBSUCCESSVALUE"	=> "Dziękujemy za zapisanie się do naszego newslettera",
	"SUBERROR"			=> "Błąd, informacje o subskrypcji ",
	"SUBERRORVALUE"		=> "Niestety nie udało Ci się zapisać do newslettera",
	"SUBVALUE"			=> "Tekst przycisku Zapisz się do newslettera ",
	"SUBVALUEVALUE"		=> "Zapisz się do Newslettera",
	"FORMSUBSCRIBE"		=> "Tekst zachęcający do subskrypcji",
	"SAVE"				=> "Zapisz",

	"PAYPAL"			=> "Wesprzyj mmnie przez paypal :) Dzięki!"

];

?>
